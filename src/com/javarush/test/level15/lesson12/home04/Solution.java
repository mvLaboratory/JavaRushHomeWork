...
public class Solution {
    ...
    public static void readKeyFromConsoleAndInitPlanet() {
        ...
        try
        {
            userPlanet = reader.readLine();
        }
        catch (IOException e) {
            e.printStackTrace();
        }

        if (userPlanet.equals(Planet.EARTH)) thePlanet = Earth.getInstance();
        else if (userPlanet.equals(Planet.MOON)) thePlanet = Moon.getInstance();
        else if (userPlanet.equals(Planet.SUN)) thePlanet = Sun.getInstance();
        else thePlanet = null;
    }
